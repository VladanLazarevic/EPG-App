...
class EPGRepositoryImpl(
            ...
            if (response.isSuccessful) {
                ...
                if (serverChannels != null) {
                    ...
                    val appChannels = ChannelMapper.mapServerListToAppList(serverChannels)
                        .map { channel -> channel.copy(isFavorite = channel.channelId in favoriteIds) }
    ...
    override suspend fun toggleFavoriteStatus(channelId: String) {
        withContext(Dispatchers.IO) {
            favoriteManager.toggleFavorite(channelId)
        }
    }
    ...
    override suspend fun getPrograms(
        ...
        // Ceo posao obavijamo u pozadinskoj IO niti.
        // Ovo uključuje petlju, sve mrežne pozive, i najvažnije,
        // parsiranje i mapiranje podataka nakon svakog poziva.
        return withContext(Dispatchers.IO) {
            val auid = auidRepository.getAuidString()
            if (auid == null) {
                Log.e(TAG, "AUID is null, cannot fetch programs.")
                // Moramo eksplicitno vratiti Result iz withContext bloka
                return@withContext Result.failure(Exception("AUID could not be retrieved for programs."))
            }
            ...
            allChannelIds.chunked(batchSize).forEachIndexed { batchIndex, channelIdChunk ->
                val channelIdsString = channelIdChunk.joinToString(separator = ",")
                Log.d(TAG, "Fetching programs for batch ${batchIndex + 1}: $channelIdsString")
                ...
                try {
                    ...
                    if (response.isSuccessful) {
                        ...
                        if (channelsWithProgramsList != null) {
                            ...
                            Log.d(TAG, "Batch ${batchIndex + 1} success.")
                        } else {
                            Log.w(TAG, "Program list body is null for batch ${batchIndex + 1}.")
                        }
                    } else {
                        val errorCode = response.code()
                        val errorMsg = response.message()
                        Log.e(TAG, "Failed to fetch programs for batch ${batchIndex + 1}. Code: $errorCode, Msg: $errorMsg")
                    }
                } catch (e: Exception) {
                    Log.e(TAG, "Exception for batch ${batchIndex + 1} when fetching programs: ${e.message}", e)
                }
            }

            if (anyBatchSucceeded || allFetchedPrograms.isNotEmpty()) {
                Log.d(TAG, "Total programs fetched across all batches: ${allFetchedPrograms.size}")
                Result.success(allFetchedPrograms)
            } else {
                Log.e(TAG, "Failed to fetch programs for any batch.")
